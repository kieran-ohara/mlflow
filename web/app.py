...
GRPC_HOST = os.getenv("GRPC_HOST", default="localhost")
GRPC_PORT = "8500"
...
def predict(request):
    ...
    npImage = np.fromstring(file.read(), np.uint8)
    imageData = cv2.imdecode(npImage, cv2.IMREAD_GRAYSCALE).astype('float32')
